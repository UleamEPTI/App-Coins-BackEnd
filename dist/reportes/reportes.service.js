...
let ReportesService = class ReportesService {
    ...
    constructor(cursoRepository, reciclajeRepository, institucionRepository) {
    ...
    async generarReporteInstitucion(institucion_id, periodo, usuarioRol, usuarioInstitucionId) {
        ...
        if (!institucion)
            throw new common_1.NotFoundException(`Institución ${institucion_id} no encontrada`);
    ...
    generarPDF(data) {
        return new Promise((resolve, reject) => {
            ...
            const formatearNumero = (valor, decimales = 0) => new Intl.NumberFormat('es-EC', {
            ...
            const dibujarTarjeta = (x, y, ancho, colorAcento, etiqueta, valor, detalle) => {
                const alto = 84;
            ...
            const dibujarFila = (y, posicion, nombre, puntos, alto, alterna) => {
            ...
            const cardY = 231;
            dibujarTarjeta(marginX, cardY, cardWidth, COLOR.verde, 'Cursos incluidos', formatearNumero(data.totalCursos), 'en el presente reporte');
            dibujarTarjeta(marginX + cardWidth + gap, cardY, cardWidth, COLOR.lima, 'Kilos reciclados', formatearNumero(data.totalKilos, 2) + ' kg', 'peso registrado en el período');
            dibujarTarjeta(marginX + (cardWidth + gap) * 2, cardY, cardWidth, COLOR.dorado, 'Puntos generados', formatearNumero(data.totalPuntos), 'puntos obtenidos en el período');
            let y = cardY + 116;
            ...
            doc
                ...
                .font('Helvetica')
                .fontSize(8.5)
                .text(formatearNumero(data.cursos.length) + ' curso' + (data.cursos.length === 1 ? '' : 's') + ' en el listado', marginX + 13, y + 18);
            y += 39;
            ...
            data.cursos.forEach((c, i) => {
                ...
                const altoTexto = Math.min(36, doc.heightOfString(c.nombre || 'Curso sin nombre', { width: nombreWidth, lineGap: 1 }));
...
exports.ReportesService = ReportesService = __decorate([
    ...
    __param(2, (0, typeorm_1.InjectRepository)(institucion_entity_1.Institucion)),
